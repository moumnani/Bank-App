...
public class EmployeeUIController {
	...
	/*
	 * @GetMapping("/employee/customerList") public String showCustomerList(Model
	 * model) { List<CustomerVO> customerVOs=customerService.findCustomers();
	 * 
	 * model.addAttribute("customerVOs", customerVOs); return
	...
	public String showCustomerList(Model model) {
	   ...
	   List<CustomerVO> customerVOs2 = new ArrayList<>();

		for (CustomerVO customer : customerVOs) {
			byte[] b = customer.getImage();
			byte[] encodeBase64 = Base64.getEncoder().encode(b);
			try {
				String base64DataString = new String(encodeBase64, "UTF-8");
				customer.setPhotoName(base64DataString);
				// photoList.add(base64DataString);
				customerVOs2.add(customer);
